...
TOOL_REGISTRY = ToolRegistry(
    [
        ...
        _spec(
            "list_files",
            "列出知识库中所有受支持的本地结构化、文本、演示、PDF 和图片文档。",
            EmptyArguments,
            list_files,
            retryable=True,
        ),
        ...
        _spec(
            "inspect_excel",
            "确定性识别一个已登记 Excel 的逐 Sheet Schema，或 CSV 的表头 Schema。",
            FileIdArguments,
            inspect_excel,
            retryable=True,
        ),
        _spec(
            "get_table_schema",
            "读取一个 Excel 或 CSV 已保存的真实表格 Schema。",
            TableSchemaArguments,
            get_table_schema,
        ),
        _spec(
            "query_table",
            "用白名单过滤、字段选择、排序和 limit 查询已解析 Excel Sheet 或 CSV。",
            QueryTableArguments,
            query_table,
            retryable=True,
        ),
        _spec(
            "aggregate_table",
            "由 Python 对已解析 Excel Sheet 或 CSV 执行 count、sum、avg、min 或 max。",
            AggregateTableArguments,
            aggregate_table,
        ),
        _spec(
        ...
        _spec(
            "retrieve_document",
            "通过文件、类型、页码或 slide 过滤，从本地非结构化文档执行混合检索并返回 Evidence。",
            RetrieveDocumentArguments,
            retrieve_document,
            retryable=True,
        ),
